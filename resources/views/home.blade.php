...
<body class="bg-pink-100 bg-cover bg-center text-center overflow-x-hidden min-h-screen flex flex-col">
  ...
  <section class="max-w-[1200px] mx-auto pt-12 px-4 sm:px-6 md:px-[60px] lg:px-[100px]">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      @php
        $categories = [
          'Donuts' => ['image' => 'treats_donut.png', 'anchor' => 'donuts'],
          'Cookies' => ['image' => 'treats_cookies.png', 'anchor' => 'cookies'],
          'Drinks' => ['image' => 'treats_drinks.png', 'anchor' => 'drinks'],
          'Meals' => ['image' => 'treats_meals.png', 'anchor' => 'meals']
        ];
      @endphp

      @foreach ($categories as $title => $data)
        <a href="{{ route('shop') }}#{{ $data['anchor'] }}" class="block">
          <div class="bg-[#FDC0D0] border-2 border-[#E55182] rounded-[20px] flex flex-col justify-between transition duration-300 ease-in-out hover:shadow-md hover:scale-[1.02]">
            <div class="aspect-[3/2] overflow-hidden rounded-t-[20px]">
              <img src="{{ asset('images/' . $data['image']) }}" alt="{{ $title }}" class="w-full h-full object-cover">
            </div>
